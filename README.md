...
Section VI → slope_compare, (long-term validation)
...
**5. Convergence check.** Before interpreting results, verify that all fitted models satisfy R̂ ≤ 1.01 and both bulk and tail ESS ≥ 400 (Vehtari et al., 2021).
...
**How the horseshoe selects.** Each interaction coefficient γ_k has a local scale λ_k \~ C+(0,1) and a global scale τ \~ C+(0,1). The shrinkage factor κ_k = 1 / (1 + λ_k²) summarizes the degree of shrinkage: κ_k ≈ 1 means the coefficient is pulled to zero (noise), κ_k ≈ 0 means it escapes shrinkage (signal).

An interaction is retained when all three criteria hold jointly:

-   95% credible interval (CrI) excludes zero

-   Posterior mean of κ_k is substantially smaller than those of the excluded interactions

-   Signal-to-noise ratio \|E\[γ_k \| Y\]\| / SD(γ_k \| Y) is sufficiently large

**Selection results.** Retained and excluded interactions form two naturally separated clusters in κ:
...
The gap between retained (κ ≤ 0.71) and excluded (κ ≥ 0.78) groups makes the selection robust to any specific numerical cutoff.
...
For a new individual with baseline eGFR y\_{i1}, we update the random effects analytically using the closed-form result of Kammer et al. (2023):

*E\[b_0i \| y\_{i1}\] = (σ²\_{b0} / (σ²\_{b0} + σ²)) · r\_{i1}*

*E\[b_1i \| y\_{i1}\] = (ρ σ\_{b0} σ\_{b1} / (σ²\_{b0} + σ²)) · r\_{i1}*

where r\_{i1} = y\_{i1} − x\_{i1}ᵀβ is the baseline residual. This update is applied draw-by-draw across all MCMC samples so that parameter uncertainty propagates into the credible intervals of personalized predictions---strictly more honest than plug-in point estimates.
...
We refit the main model under six prior configurations to assess robustness:
  ...
  **Model**      **Coefficient prior**     **Variance prior**
  ...
  2              Matched Normal            Half-t₃

  3              t₃                        Matched IG

  4              Matched Normal)           Matched IG

  5              t₃                        IG(0.01, 0.01) --- diffuse

  6              t₃                        IG(1, 1) --- mild
  ------------------------------------------------------------------------

> **Variance matching.** Exa,ple: Var\[t₃(0, 5)\] = 75, so the matched Normal uses SD = √75 ≈ 8.66, and the matched Inverse-Gamma mean is 75. Note that brms parameterizes Normal priors in SD; the code normal(0, 8.66) corresponds to N(0, 75) in the manuscript, where 75 is variance.
...
**Predicted slope.** For each subject, posterior draws of the individual slope are computed as slope_i\^(s) = β₁\^(s) + b_1i\^(s) + Z_iᵀγ\^(s). The posterior exceedance probability Pr(slope_i \< c \| Y) is then used as a continuous classifier.

**Observed slope. T**wo-point annual linear rate of change between wave 4 (baseline) and the subject\'s latest observation in waves 7--8.
...
The high NPV (99.7% and 96.7%) makes this model particularly valuable for ruling out rapid decline in screening settings. The low PPV reflects the low prevalence of rapid decliners in a healthy general population (1.7% and 8.2%), not poor discrimination: in low-prevalence settings, even strong classifiers yield low PPV because most positive predictions are false positives.
